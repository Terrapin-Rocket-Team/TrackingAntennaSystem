...
class RocketDataGenerator(BaseGenerator):
    ...
    def __init__(self, 
        ...
        :param heading_angle: Direction of rocket on 2D (Ex: North, etc.)
    ...
    def update_quaternion(self, q, angular_velocity, dt):
        """
        Update quaternion based on angular velocity using Range Kutta integration.
        ...
        """
        def f(qi):
            return self.angular_velocity_to_quaternion_derivative(qi, angular_velocity)
    ...
    def drag_force_inertial(self, conditions, relative_velocity, relative_velocity_mag, aoa):
        """
        ...
        C_d = .1 # self.total_drag_coefficient(reynolds, mach_number) --> Massive Simplification for now just want somewhat realistic data
        ...
        # C_d = C_d * multiplier - .45 # artificial modification for better data
        # print(C_d)
        drag_force = -0.5 * C_d * density * A_eff * relative_velocity_mag**2 * (relative_velocity / relative_velocity_mag) 
        return drag_force
    ...
    def total_force_and_moment_body(self, time, altitude, conditions, relative_velocity, relative_velocity_mag, rotation_matrix):
        ...
        aoa = np.pi
        if relative_velocity_mag != 0:
            v_hat = relative_velocity / relative_velocity_mag
            cos_aoa = np.dot(rotation_matrix[:, 2], -v_hat)
            aoa = np.arccos(np.clip(cos_aoa, -1, 1))  # radians
        F_drag_i = self.drag_force_inertial(conditions, relative_velocity, relative_velocity_mag, aoa)
