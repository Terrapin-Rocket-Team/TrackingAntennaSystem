...
void MotorPins::motor_init(int motor_pul, int motor_dir, double motorangle, int gear)
{
    gearRatio = gear;
    MOTOR1_PUL = motor_pul;
...
{   
   ...
   // delay(T4_PUL_LOW_US);     // t4: LOW >2.5µs
    ...
    motor_angle += ((dir) ? (1.80/((double)microSteps)) : (-1.0 * 1.80/((double)microSteps)));         // changed angle per microstep depends on the formula -> angle_per_step*step/microstep*1/Gear_ratio
    stepsOut++;
    return true;
}
...
void MotorPins::motor_set_angle(double theta, float rpm, long int stepsPerRev = 1600) {
    ...
    steps = (long int)(fabs(delta)/360.0*1600.0*(double)gearRatio); //(long int)(fabsf(delta) /360.0f * (float)stepsPerRev * (float)gearRatio);
